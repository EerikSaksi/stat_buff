...
CREATE POLICY user_exercise_select ON "user_exercise" FOR select to query_sender using (username = (select username from active_user()));
...
CREATE POLICY workout_select ON "workout" FOR select to query_sender using (username = (select username from active_user()));
