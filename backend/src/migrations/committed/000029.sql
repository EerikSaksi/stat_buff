--! Previous: sha1:38e624f24fdf10085a10cb7ef95cbbd513fa44c1
--! Hash: sha1:2fe30ab28a61f021ffef1d6e99c3f7b9726aab12
...
create POLICY workout_plan_day_insert_policy ON workout_plan_day FOR insert with check (id in (select workout_plan_day.id from workout_plan inner join workout_plan_day on workout_plan_day.workout_plan_id = workout_plan.id where app_user_id = (select current_user_id())));
drop policy if exists workout_plan_day_update_policy on workout_plan_day;
CREATE POLICY workout_plan_day_update_policy ON workout_plan_day FOR update USING (id in (select workout_plan_day.id from workout_plan inner join workout_plan_day on workout_plan_day.workout_plan_id = workout_plan.id where app_user_id = (select current_user_id())));
drop policy if exists workout_plan_day_delete_policy on workout_plan_day;
CREATE POLICY workout_plan_day_delete_policy ON workout_plan_day FOR delete USING (id in (select workout_plan_day.id from workout_plan inner join workout_plan_day on workout_plan_day.workout_plan_id = workout_plan.id where app_user_id = (select current_user_id())));
...
create POLICY workout_plan_exercise_insert_policy ON workout_plan_exercise FOR insert with check (id in (select workout_plan_exercise.id from workout_plan inner join workout_plan_day on workout_plan_day.workout_plan_id = workout_plan.id inner join workout_plan_exercise on workout_plan_day.id = workout_plan_exercise.workout_plan_day_id where app_user_id = (select current_user_id())));
drop policy if exists workout_plan_exercise_update_policy on workout_plan_exercise;
CREATE POLICY workout_plan_exercise_update_policy ON workout_plan_exercise FOR update USING (id in (select workout_plan_exercise.id from workout_plan inner join workout_plan_day on workout_plan_day.workout_plan_id = workout_plan.id inner join workout_plan_exercise on workout_plan_day.id = workout_plan_exercise.workout_plan_day_id where app_user_id = (select current_user_id())));
drop policy if exists workout_plan_exercise_delete_policy on workout_plan_exercise;
CREATE POLICY workout_plan_exercise_delete_policy ON workout_plan_exercise FOR delete USING (id in (select workout_plan_exercise.id from workout_plan inner join workout_plan_day on workout_plan_day.workout_plan_id = workout_plan.id inner join workout_plan_exercise on workout_plan_day.id = workout_plan_exercise.workout_plan_day_id where app_user_id = (select current_user_id())));
